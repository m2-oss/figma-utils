...
export async function loadImage(config: SrcImageConfig, xFigmaToken: string): Promise<Buffer | undefined> {
    ...
    if (config.scale <= 4 && config.scale >= 0.01) {
        try {
            ...
            let cacheArray: Buffer | undefined;
            if (cacheDir.length > 0) {
                try {
                    cacheArray = await fs.readFile(path.resolve(cacheDir, `${hash}.${config.format}`));
                } catch (e) {
